...
void ParameterManager::_parameterUpdate(int vehicleId, int componentId, QString parameterName, int parameterCount, int parameterId, int mavType, QVariant value)
{
    ...
    if (!_mapParameterName2Variant.contains(componentId) || !_mapParameterName2Variant[componentId].contains(parameterName)) {
        ...
        // User Hai
//        qDebug() << "Update fact!!!" << endl;
//        qDebug() << "Size: " << _mapParameterName2Variant.size() << endl;
...
{
//    QMap<int, QVariantMap>::Iterator _tmp;
//    for(_tmp = _mapParameterName2Variant.begin(); _tmp != _mapParameterName2Variant.end(); _tmp++)
//    {
//       qDebug() << *_tmp << endl;
//    }
...
//               qDebug() << *_tmp << endl;
//            }
//            qDebug() << "DOITRONG: " << _mapParameterName2Variant.size() << endl;
//    }
...
void ParameterManager::sendParameterMavlink(int componentId, const QString& paramName, const QVariant& value)
{
    mavlink_param_set_t     p;
    mavlink_param_union_t   union_value;
    ...
    FactMetaData::ValueType_t factType = FactMetaData::valueTypeFloat;
    p.param_type = _factTypeToMavType(factType);

...
const char* Joystick::_thrusterModeSettingsKey =        "ThrusterMode";
...
Joystick::Joystick(const QString& name, int axisCount, int buttonCount, int hatCount, MultiVehicleManager* multiVehicleManager)
    ...
    , _lastButtonBits(0)
    , _thrusterMode(ThrusterModeDownZero)
...
void Joystick::_setDefaultCalibration(void) {
    ...
    _rgFunctionAxis[massShifterFunction]       = 2;
    _rgFunctionAxis[pistolFunction]      = 3;
    _rgFunctionAxis[rudderFunction]        = 0;
    _rgFunctionAxis[thrusterFunction]   = 1;
...
void Joystick::_loadSettings(void)
{
    ...
    _thrusterMode = (ThrusterMode_t)settings.value(_thrusterModeSettingsKey, ThrusterModeDownZero).toInt(&convertOk);
    badSettings |= !convertOk;

    qCDebug(JoystickLog) << "_loadSettings calibrated:txmode:thrustermode:exponential:deadband:badsettings" << _calibrated << _transmitterMode << _thrusterMode << _exponential << _deadband << badSettings;
...
void Joystick::_saveSettings(void)
{
    ...
    settings.setValue(_thrusterModeSettingsKey, _thrusterMode);

    qCDebug(JoystickLog) << "_saveSettings calibrated:thrustermode:deadband:txmode" << _calibrated << _thrusterMode << _deadband << _circleCorrection << _transmitterMode;
...
uint8_t Joystick::_adjustRange_Hat(int value)
{    
    float correctedValue = static_cast<float>(value)/32767.0f;

    return static_cast<uint8_t>(std::max(0.f, std::min(correctedValue, 1.0f)));
}
...
//            qDebug() << axisIndex << " " << _rgAxisValues[axisIndex] << endl;
...
//                qDebug() << hatIndex << " " << hatButtonIndex << " " << _getHat(hatIndex,hatButtonIndex) << endl;
        ...
        if (_activeVehicle->joystickEnabled() && !_calibrationMode && _calibrated) {
            int     axis = _rgFunctionAxis[massShifterFunction];
            float   massshifter = _adjustRange(_rgAxisValues[axis], _rgCalibration[axis], _deadband);

                    axis = _rgFunctionAxis[pistolFunction];
            float   pistol = _adjustRange(_rgAxisValues[axis], _rgCalibration[axis], _deadband);

                    axis = _rgFunctionAxis[rudderFunction];
            float   rudder = _adjustRange(_rgAxisValues[axis], _rgCalibration[axis],_deadband);

                    axis = _rgFunctionAxis[thrusterFunction];
            float   thruster = _adjustRange(_rgAxisValues[axis], _rgCalibration[axis], _thrusterMode==ThrusterModeDownZero?false:_deadband);

                    axis = 4;
            uint8_t   vlefthat = _adjustRange_Hat(_rgAxisValues[axis]);

                    axis = 5;
            uint8_t   vrighthat = _adjustRange_Hat(_rgAxisValues[axis]);

            if ( _accumulator ) {
                static float thruster_accu = 0.f;

                thruster_accu += thruster*(40/1000.f); //for thruster to change from min to max it will take 1000ms (40ms is a loop time)

                thruster_accu = std::max(static_cast<float>(-1.f), std::min(thruster_accu, static_cast<float>(1.f)));
                thruster = thruster_accu;
            }

            if ( _circleCorrection ) {
                float massshifter_limited = std::max(static_cast<float>(-M_PI_4), std::min(massshifter, static_cast<float>(M_PI_4)));
                float pistol_limited = std::max(static_cast<float>(-M_PI_4), std::min(pistol, static_cast<float>(M_PI_4)));
                float rudder_limited = std::max(static_cast<float>(-M_PI_4), std::min(rudder, static_cast<float>(M_PI_4)));
                float thruster_limited = std::max(static_cast<float>(-M_PI_4), std::min(thruster, static_cast<float>(M_PI_4)));

                // Map from unit circle to linear range and limit
                massshifter =      std::max(-1.0f, std::min(tanf(asinf(massshifter_limited)), 1.0f));
                pistol =     std::max(-1.0f, std::min(tanf(asinf(pistol_limited)), 1.0f));
                rudder =       std::max(-1.0f, std::min(tanf(asinf(rudder_limited)), 1.0f));
                thruster =  std::max(-1.0f, std::min(tanf(asinf(thruster_limited)), 1.0f));
            }
            ...
            if ( _exponential != 0 ) {
                ...
                massshifter =      -_exponential*powf(massshifter,3) + (1+_exponential)*massshifter;
                pistol =     -_exponential*powf(pistol,3) + (1+_exponential)*pistol;
                rudder =       -_exponential*powf(rudder,3) + (1+_exponential)*rudder;
            }

            // Adjust thruster to 0:1 range
            if (_thrusterMode == ThrusterModeCenterZero && _activeVehicle->supportsThrusterModeCenterZero()) {
                if (!_activeVehicle->supportsNegativeThrust() || !_negativeThrust) {
                    thruster = std::max(0.0f, thruster);
                }
            } else {
                thruster = (thruster + 1.0f) / 2.0f;
            }
            ...
            qCDebug(JoystickValuesLog) << "name:massshifter:pistol:rudder:thruster" << name() << massshifter << -pistol << rudder << thruster;

//            qDebug() << "Debug Call" << endl;

            // NOTE: The buttonPressedBits going to MANUAL_CONTROL are currently used by ArduSub.
            emit manualControl(massshifter, -pistol, rudder, thruster, vlefthat, vrighthat, buttonPressedBits, _activeVehicle->joystickMode());
        }
...
int Joystick::thrusterMode(void)
{
    return _thrusterMode;
}

void Joystick::setThrusterMode(int mode)
{
    if (mode < 0 || mode >= ThrusterModeMax) {
        qCWarning(JoystickLog) << "Invalid thruster mode" << mode;
        return;
    }

    _thrusterMode = (ThrusterMode_t)mode;

    if (_thrusterMode == ThrusterModeDownZero) {
        setAccumulator(false);
    }

    _saveSettings();
    emit thrusterModeChanged(_thrusterMode);
}

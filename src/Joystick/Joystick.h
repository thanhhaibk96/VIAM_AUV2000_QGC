...
public:
    ...
    typedef enum {
        ThrusterModeCenterZero,
        ThrusterModeDownZero,
        ThrusterModeMax
    } ThrusterMode_t;
    ...
    Q_PROPERTY(int thrusterMode READ thrusterMode WRITE setThrusterMode NOTIFY thrusterModeChanged)
...
*/
    ...
    int thrusterMode(void);
    void setThrusterMode(int mode);
...
signals:
    ...
    void thrusterModeChanged(int mode);
    ...
    ///     @param mass-shifter     Range is -1:1, negative meaning mass-shifter at tail, positive meaning mass-shifter head
    ///     @param pistol    Range i -1:1, negative meaning pistol at tail, positive meaning pistol at head
    ///     @param rudder      Range is -1:1, negative meaning rudder left, positive meaning rudder right
    ///     @param thrster Range is 0:1, 0 meaning no thruster, 1 meaning full thruster
    ///     @param mode     See Vehicle::JoystickMode_t enum
    void manualControl(float _mass_shifter, float _pistol, float _rudder, float _thruster, quint8 _vlefthat, quint8 _vrighthat, quint16 buttons, int joystickMmode);
...
protected:
    ...
    uint8_t   _adjustRange_Hat(int value);
...
private:
    ...
    static const char* _thrusterModeSettingsKey;

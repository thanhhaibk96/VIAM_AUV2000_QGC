...
UAS::UAS(MAVLinkProtocol* protocol, Vehicle* vehicle, FirmwarePluginManager * firmwarePluginManager) : UASInterface(),
    ...
    controlMassShifterManual(true),
    controlPistolManual(true),
    controlRudderManual(true),
    controlThrustManual(true),
    manualMassShifterAngle(0),
    manualPistolAngle(0),
    manualRudderAngle(0),
    manualThrust(0),
...
*/
void UAS::setExternalControlSetpoint(float mass_shifter, float pistol, float rudder, float thrust, quint8 vlefthat, quint8 vrighthat, quint16 buttons, int joystickMode)
{
    if (!_vehicle) {
        return;
    }
    ...
    static float manualMassShifter = 0.0;
    static float manualPistol = 0.0;
    static float manualRudderAngle = 0.0;
    static float manualThrust = 0.0;
    static quint8 manualVLeftHat = 0.0;
    static quint8 manualVRightHat = 0.0;
    ...
    } else if ((!qIsNaN(mass_shifter) && mass_shifter != manualMassShifter) || (!qIsNaN(pistol) && pistol != manualPistol) ||
             (!qIsNaN(rudder) && rudder != manualRudderAngle) || (!qIsNaN(thrust) && thrust != manualThrust) ||
    ...
    if (sendCommand) {
        ...
        manualMassShifter = mass_shifter;
        manualPistol = pistol;
        manualRudderAngle = rudder;
        manualThrust = thrust;
        ...
        if (joystickMode == Vehicle::JoystickModeAttitude) {
            ...
            mavlink_euler_to_quaternion(mass_shifter, pistol, rudder, attitudeQuaternion);
            ...
            float dcm[3][3];
            mavlink_euler_to_dcm(mass_shifter, pistol, rudder, dcm);
            ...
            vy += mass_shifter;
            vz -= 2.0f*(thrust-0.5);
            yawrate += rudder; //XXX: not sure what scale to apply here
            ...
            manualThrust = thrust;
            manualButtons = buttons;
            manualVLeftHat = vlefthat;
            manualVRightHat = vrighthat;

            // Store scaling values for all 3 axes
            const float axesScaling = 1.0 * 1000.0;

            // Calculate the new commands for roll, pitch, yaw, and thrust
            const float newMassShifterCommand = mass_shifter * axesScaling;
            // negate pitch value because pitch is negative for pitching forward but mavlink message argument is positive for forward
            const float newPistolCommand = -pistol * axesScaling;
            const float newRudderCommand = rudder * axesScaling;
            const float newThrustCommand = thrust * axesScaling;
            const uint8_t newVLeftHatCommand = vlefthat;
            const uint8_t newVRightHatCommand = vrighthat;

//            qDebug() << newMassShifterCommand << newPistolCommand << newRudderCommand << newThrustCommand;
            ...
            mavlink_msg_manual_control_pack_chan(mavlink->getSystemId(),
                                                 ...
                                                 &message,
                                                 this->uasId,
                                                 newMassShifterCommand, newPistolCommand, newRudderCommand, newThrustCommand,
                                                 newVLeftHatCommand, newVRightHatCommand, buttons);
        }

//        qDebug() << message.checksum << " " << message.len << " " << message.payload64[0] << endl;

...
const char* Vehicle::_battery1FactGroupName =           "batteryHead";
//const char* Vehicle::_battery2FactGroupName =           "batteryMotor";
const char* Vehicle::_battery3FactGroupName =           "batteryTail";
const char* Vehicle::_windFactGroupName =               "wind";
const char* Vehicle::_vibrationFactGroupName =          "vibration";
const char* Vehicle::_boardStatusFactGroupName =        "boardstatus";
const char* Vehicle::_altimeterFactGroupName =          "altimeter";
...
//    , _battery2FactGroup(this)
    , _battery3FactGroup(this)
    , _windFactGroup(this)
    , _vibrationFactGroup(this)
    , _altimeterFactGroup(this)
    , _boardStatusFactGroup(this)
...
//    _addFactGroup(&_battery2FactGroup,          _battery2FactGroupName);
    _addFactGroup(&_battery3FactGroup,          _battery3FactGroupName);
    _addFactGroup(&_altimeterFactGroup,          _altimeterFactGroupName);
//    _addFactGroup(&_boardStatusFactGroup,       _boardStatusFactGroupName);
...
void Vehicle::sendDutyCycle_MassShifter(float _duty_cycle)
{
    parameterManager()->sendParameterMavlink(1, "mass_shifter_DUTY_CYCLE", _duty_cycle);
}
...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    case MAVLINK_MSG_ID_BATTERY_STATUS:
        _handleBatteryHeadStatus(message);
        break;
    ...
    case MAVLINK_MSG_ID_SERIAL_CONTROL:
    {
        ...
        break;
    /// My_AUV2000 IDs
    case MAVLINK_MSG_ID_CUSTOM_MSG_PISTOL:
    {
        _handlePistol(message);
        break;
    }
    case MAVLINK_MSG_ID_CUSTOM_MSG_MASS_SHIFTER:
    {
        _handleMassShifter(message);
        break;
    }
    case MAVLINK_MSG_ID_CUSTOM_MSG_THRUSTER:
    {
        _handleThruster(message);
        break;
    }
    case MAVLINK_MSG_ID_CUSTOM_MSG_BOARD_ARM1:
    {
        _handleBoardARM1(message);
        break;
    }
    case MAVLINK_MSG_ID_CUSTOM_MSG_BOARD_ARM2:
    {
...
void Vehicle::_handleAltitude(mavlink_message_t& message)
{
    ...
    if (!_globalPositionIntMessageAvailable) {
        ...
        if (!_gpsRawIntMessageAvailable) {
            _altitudeAMSLFact.setRawValue((altitude.altitude_amsl-1)*10);
        }
    }
}
...
void Vehicle::_handleBoardStatus(mavlink_message_t& message)
{
//    mavlink_vibration_t vibration;
//    mavlink_msg_vibration_decode(&message, &vibration);

      _boardStatusFactGroup.ARM1()->setRawValue(0);
      _boardStatusFactGroup.ARM2()->setRawValue(0);
      _boardStatusFactGroup.VIAM_NAVI()->setRawValue(0);
      _boardStatusFactGroup.PISTOL()->setRawValue(0);
      _boardStatusFactGroup.MASS_SHIFTER()->setRawValue(0);
      _boardStatusFactGroup.THRUSTER()->setRawValue(0);
}

//void Vehicle::_handleAltimeter(mavlink_message_t& message)
//{
...
//    if (sysStatus.current_battery_head == -1) {
...
//        _battery1FactGroup.current()->setRawValue((float)sysStatus.current_battery_head / 100.0f);
//        _battery1FactGroup.current()->setRawValue((float)sysStatus.current_battery_head / 100.0f);
//    }
//    if (sysStatus.voltage_battery_head == UINT16_MAX) {
//        _battery1FactGroup.voltage()->setRawValue(VehicleBatteryFactGroup::_voltageUnavailable);
//    } else {
//        _battery1FactGroup.voltage()->setRawValue((double)sysStatus.voltage_battery_head / 1000.0);
//        // current_battery is 10 mA and voltage_battery is 1mV. (10/1e3 times 1/1e3 = 1/1e5)
//        _battery1FactGroup.instantPower()->setRawValue((float)(sysStatus.current_battery_head*sysStatus.voltage_battery_head)/(100000.0));
//    }
//    _battery1FactGroup.percentRemaining()->setRawValue(sysStatus.battery_remaining_head);

//    if (sysStatus.battery_remaining_head > 0) {
//        if (sysStatus.battery_remaining_head < _settingsManager->appSettings()->batteryPercentRemainingAnnounce()->rawValue().toInt() &&
//                sysStatus.battery_remaining_head < _lastAnnouncedLowBatteryPercent) {
//            _say(QString(tr("%1 low battery: %2 percent remaining")).arg(_vehicleIdSpeech()).arg(sysStatus.battery_remaining_head));
//        }
//        _lastAnnouncedLowBatteryPercent = sysStatus.battery_remaining_head;
//    }

    if (sysStatus.current_battery == -1) {
        _battery1FactGroup.current()->setRawValue(VehicleBatteryFactGroup::_currentUnavailable);
    } else {
        _battery1FactGroup.current()->setRawValue((float)sysStatus.current_battery);
    }
    if (sysStatus.voltage_battery == UINT16_MAX) {
        _battery1FactGroup.voltage()->setRawValue(VehicleBatteryFactGroup::_voltageUnavailable);
    } else {
        _battery1FactGroup.voltage()->setRawValue((double)sysStatus.voltage_battery);
        _battery1FactGroup.instantPower()->setRawValue((float)(sysStatus.current_battery*sysStatus.voltage_battery/1000.0f)/(1000.0));
    }
    ...
    _battery1FactGroup.totalBattery()->setRawValue(sysStatus.batery_total);
    _battery1FactGroup.usedBattery()->setRawValue(sysStatus.batery_used);
    _battery1FactGroup.mahConsumed()->setRawValue(sysStatus.batery_capacity);

//    if (sysStatus.current_battery_tail == -1) {
//        _battery3FactGroup.current()->setRawValue(VehicleBatteryFactGroup::_currentUnavailable);
//    } else {
//        _battery3FactGroup.current()->setRawValue((float)sysStatus.current_battery_tail);
//    }
//    if (sysStatus.voltage_battery_tail == UINT16_MAX) {
//        _battery3FactGroup.voltage()->setRawValue(VehicleBatteryFactGroup::_voltageUnavailable);
//    } else {
//        _battery3FactGroup.voltage()->setRawValue((double)sysStatus.voltage_battery_tail);
//        _battery3FactGroup.instantPower()->setRawValue((float)(sysStatus.current_battery_tail*sysStatus.voltage_battery_tail/1000.0f)/(1000.0));
//    }
//    _battery3FactGroup.percentRemaining()->setRawValue(sysStatus.battery_remaining_tail);
//    if (sysStatus.battery_remaining_tail > 0) {
//        if (sysStatus.battery_remaining_tail < _settingsManager->appSettings()->batteryPercentRemainingAnnounce()->rawValue().toInt() &&
//                sysStatus.battery_remaining_tail < _lastAnnouncedLowBatteryPercent) {
//            _say(QString(tr("%1 low battery: %2 percent remaining")).arg(_vehicleIdSpeech()).arg(sysStatus.battery_remaining_tail));
//        }
//        _lastAnnouncedLowBatteryPercent = sysStatus.battery_remaining_tail;
//    }
//    _battery3FactGroup.totalBattery()->setRawValue(sysStatus.batery_total_tail);
//    _battery3FactGroup.usedBattery()->setRawValue(sysStatus.batery_used_tail);
//    _battery3FactGroup.mahConsumed()->setRawValue(sysStatus.batery_capacity_tail);
...
void Vehicle::_handleBatteryHeadStatus(mavlink_message_t& message)
{
    mavlink_battery_status_t bat_status;
    mavlink_msg_battery_status_decode(&message, &bat_status);

    VehicleBatteryFactGroup& batteryFactGroup = bat_status.id == 0 ? _battery1FactGroup : _battery1FactGroup;
...
//    _temperatureFactGroup.temperature2()->setRawValue(pressure.temperature / 100.0);
 //   _altitudeRelativeFact.setRawValue(-10.0*static_cast<double>(pressure.press_abs)*1e3/1e5);
//    qDebug() << "SCALE PRESSURE CALL" << endl;
}
...
///My_AUV2000 Handles_Main
void Vehicle:: _handleMassShifter(mavlink_message_t& message)
{
//    qDebug() << "MASS SHIFTER CALL" << endl;
    mavlink_custom_msg_mass_shifter_t mass_shifter;
    mavlink_msg_custom_msg_mass_shifter_decode(&message, &mass_shifter);
//    Vehicle::setSliderMassShifter(massshifter.position);

    _mass_shifterPosition = mass_shifter.position*100;
...
void Vehicle:: _handlePistol(mavlink_message_t& message)
{
    mavlink_custom_msg_pistol_t pistol;
    mavlink_msg_custom_msg_pistol_decode(&message, &pistol);
//    Vehicle::setSliderPistol(pistol.position);

    _pistolPosition = pistol.position*100;
    emit pistolPositionChanged(_pistolPosition);

    _pistolDuty = pistol.motor_duty;
    emit pistolDutyChanged(_pistolDuty);

    _pistolDSpeed = pistol.motor_dspeed;
     emit pistolDSpeedChanged(_pistolDSpeed);

    _pistolRSpeed = pistol.motor_rspeed;
     emit pistolRSpeedChanged(_pistolRSpeed);

    _pistolCurrent = pistol.motor_current;
     emit pistolCurrentChanged(_pistolCurrent);

    _pistolTempOnChip = pistol.motor_temp_on_chip;
     emit pistolTempOnChipChanged(_pistolTempOnChip);

    _pistolTempAmbient = pistol.motor_temp_ambient;
     emit pistolTempAmbientChanged(_pistolTempAmbient);
}

void Vehicle::_handleThruster(mavlink_message_t& message)
{
    mavlink_custom_msg_thruster_t thruster;
    mavlink_msg_custom_msg_thruster_decode(&message, &thruster);
...
void Vehicle::_handleBoardARM1(mavlink_message_t& message)
{
    ...
    _altimeterFactGroup.Altimeter_inFathoms()->setRawValue(board_arm1.altimeter_in_fathoms);
}

void Vehicle::_handleBoardARM2(mavlink_message_t& message)
{
    mavlink_custom_msg_board_arm2_t board_arm2;
    mavlink_msg_custom_msg_board_arm2_decode(&message, &board_arm2);
    ...
    _temperatureFactGroup.temperature2()->setRawValue(board_arm2.keller_pa3_temperature);
    _altitudeRelativeFact.setRawValue(static_cast<double>((board_arm2.keller_pa3_pressure - 1)*10));
}
...
void Vehicle::virtualTabletJoystickValue(double roll, double pitch, double yaw, double thrust)
{
    ...
    if ( !_joystickEnabled && !_highLatencyLink) {
        _uas->setExternalControlSetpoint(roll, pitch, yaw, thrust, 0, 0, 0, JoystickModeRC);
    }
}
...
bool Vehicle::supportsThrusterModeCenterZero(void) const
{
    return _firmwarePlugin->supportsThrusterModeCenterZero();
}
...
void Vehicle::setSliderMassShifter(float _value)
{
    if(_value != _sliderMassShifter)
    {
        _sliderMassShifter = _value;
        qDebug() << "Qt Slider Changed: " + QString::number(_sliderMassShifter) << endl;
        emit sliderMassShifterChanged(_sliderMassShifter);
    }
}

void Vehicle::setSliderPistol(float _value)
{
    if(_value != _sliderPistol)
    {
        _sliderPistol = _value;
        qDebug() << "Qt Slider Changed: " + QString::number(_sliderPistol) << endl;
        emit sliderPistolChanged(_sliderPistol);
    }
}
...
const char* VehicleBatteryFactGroup::_totalBatteryFactName =                "totalBattery";
const char* VehicleBatteryFactGroup::_usedBatteryFactName =                 "usedBattery";
...
const int    VehicleBatteryFactGroup::_totalBatteryUnavailable =      -1;
const int    VehicleBatteryFactGroup::_usedBatteryUnavailable =       -1;
...
VehicleBatteryFactGroup::VehicleBatteryFactGroup(QObject* parent)
    ...
    , _totalBatteryFact             (0, _totalBatteryFactName,              FactMetaData::valueTypeInt32)
    , _usedBatteryFact              (0, _usedBatteryFactName,               FactMetaData::valueTypeInt32)
    ...
    , _chargeStateFact              (0, _chargeStateFactName,               FactMetaData::valueTypeUint8)
{
    ...
    _addFact(&_totalBatteryFact,            _totalBatteryFactName);
    _addFact(&_usedBatteryFact,             _usedBatteryFactName);
    ...
    _totalBatteryFact.setRawValue       (_totalBatteryUnavailable);
    _usedBatteryFact.setRawValue        (_usedBatteryUnavailable);
...
const char* VehicleAltimeterFactGroup::_Altimeter_inFathomsFactName =            "altimeter_Fathoms";
const char* VehicleAltimeterFactGroup::_Altimeter_inMetresFactName =            "altimeter_Metres";
const char* VehicleAltimeterFactGroup::_Altimeter_inFeetFactName =       "altimeter_Feet";

VehicleAltimeterFactGroup::VehicleAltimeterFactGroup(QObject* parent)
    : FactGroup(1000, ":/json/src/Vehicle/AltimeterFact.json", parent)
    , _Altimeter_inFeetFact        (0, _Altimeter_inFeetFactName,         FactMetaData::valueTypeFloat)
    , _Altimeter_inMetresFact        (0, _Altimeter_inMetresFactName,                   FactMetaData::valueTypeFloat)
    , _Altimeter_inFathomsFact        (0, _Altimeter_inFathomsFactName,                   FactMetaData::valueTypeFloat)
{
    _addFact(&_Altimeter_inFathomsFact,       _Altimeter_inFathomsFactName);
    _addFact(&_Altimeter_inMetresFact,       _Altimeter_inMetresFactName);
    _addFact(&_Altimeter_inFeetFact,       _Altimeter_inFeetFactName);

    // Start out as not available "--.--"
    _Altimeter_inFathomsFact.setRawValue(std::numeric_limits<float>::quiet_NaN());
    _Altimeter_inMetresFact.setRawValue(std::numeric_limits<float>::quiet_NaN());
    _Altimeter_inFeetFact.setRawValue(std::numeric_limits<float>::quiet_NaN());
}

const char* VehicleBoardStatusFactGroup::_ARM1FactName =            "ARM1";
const char* VehicleBoardStatusFactGroup::_ARM2FactName =            "ARM2";
const char* VehicleBoardStatusFactGroup::_VIAM_NAVIFactName =       "VIAM_NAVI";
const char* VehicleBoardStatusFactGroup::_PISTOLFactName =          "PISTOL";
const char* VehicleBoardStatusFactGroup::_MASS_SHIFTERFactName =    "MASS_SHIFTER";
const char* VehicleBoardStatusFactGroup::_THRUSTERFactName =         "THRUTER";

VehicleBoardStatusFactGroup::VehicleBoardStatusFactGroup(QObject* parent)
    : FactGroup(1000, ":/json/src/Vehicle/BoardStatusFact.json", parent)
    , _ARM1Fact        (0, _ARM1FactName,                   FactMetaData::valueTypeBool)
    , _ARM2Fact        (0, _ARM2FactName,                   FactMetaData::valueTypeBool)
    , _VIAM_NAVIFact        (0, _VIAM_NAVIFactName,         FactMetaData::valueTypeBool)
    , _PISTOLFact        (0, _PISTOLFactName,               FactMetaData::valueTypeBool)
    , _MASS_SHIFTERFact        (0, _MASS_SHIFTERFactName,   FactMetaData::valueTypeBool)
    , _THRUSTERFact        (0, _THRUSTERFactName,           FactMetaData::valueTypeBool)
{
//    _addFact(&_ARM1Fact,       _ARM1FactName);
//    _addFact(&_ARM2Fact,       _ARM2FactName);
//    _addFact(&_VIAM_NAVIFact,       _VIAM_NAVIFactName);
//    _addFact(&_PISTOLFact,  _PISTOLFactName);
//    _addFact(&_MASS_SHIFTERFact,  _MASS_SHIFTERFactName);
//    _addFact(&_mass_shifterFact,  _mass_shifterFactName);

    // Start out as not available "--.--"
    _ARM1Fact.setRawValue(std::numeric_limits<bool>::quiet_NaN());
    _ARM2Fact.setRawValue(std::numeric_limits<bool>::quiet_NaN());
    _VIAM_NAVIFact.setRawValue(std::numeric_limits<bool>::quiet_NaN());
    _PISTOLFact.setRawValue(std::numeric_limits<bool>::quiet_NaN());
    _MASS_SHIFTERFact.setRawValue(std::numeric_limits<bool>::quiet_NaN());
    _THRUSTERFact.setRawValue(std::numeric_limits<bool>::quiet_NaN());
}

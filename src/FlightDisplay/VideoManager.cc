...
VideoManager::setToolbox(QGCToolbox *toolbox)
{
   ...
   connect(_videoSettings->webServerUrl(),  &Fact::rawValueChanged, this, &VideoManager::_webServerUrlChanged);
...
//-----------------------------------------------------------------------------
void
VideoManager::_webServerUrlChanged()
{
    _restartVideo();
}
...
VideoManager::hasVideo()
{
    ...
    QString videoSource = _videoSettings->videoSource()->rawValue().toString();
    return !videoSource.isEmpty() && videoSource != VideoSettings::videoSourceNoVideo && videoSource != VideoSettings::videoDisabled && videoSource != VideoSettings::videoSourceWebServer;
}
...
VideoManager::_updateSettings()
{
        ...
        if(pInfo) {
            switch(pInfo->type()) {
                ...
                case VIDEO_STREAM_TYPE_RTPUDP:
                    _videoReceiver->setUri(QStringLiteral("udp://127.0.0.1:%1").arg(pInfo->uri()));
                    break;
                case VIDEO_STREAM_TYPE_MPEG_TS_H264:
                    _videoReceiver->setUri(QStringLiteral("mpegts://127.0.0.1:%1").arg(pInfo->uri()));
                    break;
                default:
                    _videoReceiver->setUri(pInfo->uri());
                    break;
    ...
    if (source == VideoSettings::videoSourceUDP)
        ...
        _videoReceiver->setUri(QStringLiteral("mpegts://127.0.0.1:%1").arg(_videoSettings->udpPort()->rawValue().toInt()));
